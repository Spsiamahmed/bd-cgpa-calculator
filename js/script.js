...
function initializeForm() {
  ...
  // NEW: Real-time calculation on any input change
  form.addEventListener("input", function (e) {
    // Only trigger if we have valid inputs to show real-time feedback
    ...
    // Show preview calculation if name and credit are provided
    if (name && credit && credit > 0 && letterGrade) {
      updateResults();
    } else if (subjects.length > 0) {
      // Still update to show current totals even if form is incomplete
      updateResults();
    }
  });
...
function printSummary() {
  const printContent = `
        ...
        <body>
            ...
            <p><em>Generated by BD CGPA Calculator - bgcalculator2026</em></p>
        </body>
        </html>
    `;
...
Source: BD CGPA Calculator - bdcgpacalculator2026
    `.trim();
